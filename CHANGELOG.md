...
- Windowsインストール後の操作を3画面に収めるため、HTTPS証明書、Windows自動
  プロキシー、ログオン時自動起動を明示選択できる初回起動ウィザード試作と、
  適用失敗時に設定ファイルとOS変更を復元する処理を追加した。
- 現行環境を汚染せず初回セットアップを検証できるよう、全画面操作と
  ファイルロールバックをフェイク環境で確認するテスト、および一時CIでCA、
  プロキシー、自動起動の実適用と完全復元を確認する試験を追加した。
- JavaやPowerShell 7を利用者が事前導入せずに済むよう、専用Javaランタイム、
  単一の製品ランチャーを含むWindowsアプリイメージとMSIの試作生成を追加した。
...
- 初回起動ウィザードを画面のない環境でも利用できるよう、GUIと同じ適用・
