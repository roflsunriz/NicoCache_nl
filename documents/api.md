...
動画・音声には完成済みキャッシュが必要である。コメント保存は対象の視聴ページを
NicoCache_nl自身が取得し、得られた現行NVComment情報でコメントサーバーへ通信する。
ブラウザーがニコニコ動画へ送った直近のログインCookieは、同じ利用者の権限で視聴ページを
取得できるようプロセス内メモリーへ最大12時間だけ保持して公式ホストへ再送する。
Cookieは設定、ログ、キャッシュ、API応答には保存しない。認証情報がなくても公開動画は
ゲストとして取得し、認証必須動画で情報を取得できない場合は構造化エラーを返す。
