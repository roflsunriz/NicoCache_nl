...
プロセスの寿命は独立している。`--headless --stop`と起動管理画面の「本体を停止」は
NicoCache_nl本体だけを停止し、既存の常駐ランチャーと診断アプリを終了しない。
診断アプリは本体の次回起動を待機し、自動再起動は行わない。ウィンドウの×はトレイ格納、
「ランチャーのみ終了」は本体と診断アプリを残したままランチャーだけを終了する。
この組み合わせはWindowsアプリイメージ試験で、停止前後のランチャーPIDと診断PIDが
生存していることまで検証する。
