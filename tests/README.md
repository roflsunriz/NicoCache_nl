...
ブラウザーAPI呼出規約、動画切替処理、管理サイトのキャッシュ正規化・検索・絞り込み・並び替え、キャッシュ品質の
リンク色・アイコン表示を検証する。
...
本体停止とランチャー終了が互いの終了処理を呼ばないライフサイクル契約、認証済み
`--launcher-only-stop`の別プロセス操作、不正トークン拒否、本体継続を検証する。
...
動画ID・タイトルの保持を生成ファイルで検証する。クラッシュ後に診断プロセスが残ること、
直JARで再起動した本体の正常停止時に診断プロセスも終了することも実PIDで検証する。
これらに加え、NicoCacheGUIの
...
含まず、JARと同梱JREから起動することも検証する。ランチャーだけでは診断アプリを起動せず、
本体起動時に診断アプリも起動すること、別のランチャープロセスから`--headless --stop`を
実行すると本体PIDと診断PIDが終了して既存ランチャーPIDだけが生存すること、
`--launcher-only-stop`では逆にランチャーだけが終了することも確認する。
また管理API断を故障注入し、同梱JREの
