...
`05_topBarFilter.txt`がトップ、動画トップ、ランキング、新着、検索、タグ、視聴、ユーザーの
全HTMLへ`05_nicocache_menu.js`を挿入することも確認する。
...
非ログイン・ログインの双方で、トップ、動画トップ、ランキング、新着、検索、タグ、視聴、
ユーザーの8ルートについて、両スクリプトの自動読込、公式CommonHeaderの生成、両メニューの
`account`配置と座標順を確認した。初期調査で一部ページの公式ルートが生成されなかった原因は、
filter-matome API状態メニューが空の`#CommonHeader`を先に変更する競合と、測定タブをbackgroundで
作成して公式クライアント描画を抑制したことだった。どちらも修正し、前景タブで再測定した。
