...
変更前へ戻す。新版への更新中は復元しない。復元に失敗した場合はアンインストール
...
GitHub Actions以外での実行を拒否する。MSI試験では初回セットアップでCA、
自動プロキシー、自動起動を実際に適用し、アンインストールだけで試験前の
OS状態へ完全に戻ることを確認する。更新試験の`1.0.0`と`1.0.1`はMSIの
