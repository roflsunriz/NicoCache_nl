...
既定の`%LOCALAPPDATA%\NicoCache_nl`からインストール先を変更できる。
1.3.0の誤った既定先`%LOCALAPPDATA%\Programs\NicoCache_nl`から更新する場合だけ、
アプリ設定を正しい既定先へ移して旧フォルダーを削除する。利用者が明示した別の
インストール先は更新後も保持する。
スタートメニューに加えて
...
サイト証明書の対象は`NicoCacheCA.jar`と初回ウィザードが共通で参照する
...
ここへ保存する。
MSI更新時はアプリ側の`config.properties`と利用者データを保持し、通常の
アンインストール時はアプリ側の設定だけを削除して利用者データを残す。

既存ユーザーが旧アプリケーションフォルダーから「ドキュメント」内の新しい
ユーザーデータルートへ移行する場合は、起動管理GUIの「データルート診断」を使う。
初回セットアップ用フォルダー、TLSストア、MitM証明書と対象一覧、PACファイル、読み書き
権限、旧レイアウトの混在を項目別に表示する。パッケージの初回セットアップを完了した
新規インストールでは初回セットアップ記録が作成されるため、この確認は通常不要である。
診断の詳細とヘッドレス実行は[ユーザーデータルートの診断と移行](../../documents/user-data-root.md)
を参照する。
...
更新試験の`1.0.0`と`1.0.1`はMSIの版比較を発生させる固定テスト値であり、
パッケージ内容は実行時にチェックアウトした現在のソースから両方とも再コンパイルする。
旧版fixtureだけは1.3.0の誤った既定先を再現する。`config.properties`がある場合と
未起動でまだない場合の正しい既定先への移行に加え、明示した任意のインストール先を
維持する場合を別々に検証する。

初回ウィザードの画面遷移、選択連動、キャンセル、設定作成、既存ファイル保全、
失敗ロールバックはフェイクOS環境でローカル検証する。自動プロキシーと
ログオン時起動の実適用と完全復元は一時GitHub Actionsランナーで検証する。
タスクスケジューラーの実適用は`test-windows-task-scheduler.ps1`が一意なタスクを
登録・XML照会・更新・削除し、削除後の残存も確認する。Windowsのタスクルート直下へ
登録するため、事前に`NicoCache_nl`サブフォルダーを作成する必要はない。
...
`NicoCache_nl-<version>.msi` とSHA-256ファイルは、ZIPと同じ
