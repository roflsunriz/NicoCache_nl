...
Windowsパッケージ版は、既定でWindowsの「ドキュメント」内の
`NicoCache_nl` を利用者データルートにする。設定、`local/`、`nlFilters/`、
Extension、キャッシュ、証明書、初回セットアップ状態をここへ保存する。
アプリ本体にある既定設定、セットアップスクリプト、ネイティブライブラリは
コピーせず、その時点のインストール内容を参照する。

旧配置に利用者ファイルがある場合、新配置に同名の項目がないときだけ原子的に
コピーする。移行元、既存の移行先、明示された絶対キャッシュパスは変更しない。
環境変数 `NICOCACHE_DATA_ROOT` またはJavaシステムプロパティ
`nicocache.dataRoot` で保存先を上書きできる。実行ファイル横の
`portable.flag` は、明示ルートがない場合に従来の一体配置を選ぶ。
